...
# Module outputs (database, etc.) are appended to this file by
# `dialed:add-module` when the corresponding module is installed.
# Adding them here by default doesn't work — Terraform statically
# checks `module.NAME` references even inside try().
